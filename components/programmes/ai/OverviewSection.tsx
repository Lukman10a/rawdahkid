...
import { CheckCircle, Compass, Lightbulb } from "lucide-react";
      ...
      <div className="grid lg:grid-cols-2 gap-16 items-start">
        ...
        >
          ...
          <div className="space-y-4 text-midnight/80 dark:text-muted font-sans leading-relaxed text-lg">
            <p>{t("t9")}</p>
            <p>{t("t10")}</p>
            <p className="font-semibold text-midnight dark:text-cream italic border-s-4 border-cyan-600 dark:border-cyan-400 ps-4 py-1">
              {t("t11")}
            </p>
          </div>
        </motion.div>
        ...
        >
          <h2 className="font-cinzel text-2xl text-midnight dark:text-cream mb-6 flex items-center gap-3">
            <Lightbulb className="w-7 h-7 text-cyan-600 dark:text-cyan-400" />
            {t("t12")}
          </h2>
          <ul className="space-y-6">
            {resources.map((resource, i) => (
              <li key={i} className="flex items-start gap-4">
                <CheckCircle className="w-6 h-6 text-cyan-600 dark:text-cyan-400 shrink-0 mt-1" />
                <div>
