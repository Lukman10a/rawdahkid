...
import { CheckCircle, Compass, BookMarked } from "lucide-react";
      ...
      <div className="grid lg:grid-cols-2 gap-16 items-start">
        ...
        >
          ...
          <div className="space-y-4 text-midnight/80 dark:text-muted font-sans leading-relaxed text-lg">
            <p>{t("t9")}</p>
            <p>{t("t10")}</p>
            <p className="font-semibold text-midnight dark:text-cream italic border-s-4 border-amber-600 dark:border-amber-400 ps-4 py-1">
              {t("t11")}
            </p>
          </div>
        </motion.div>
          ...
          <ul className="space-y-6">
            {resources.map((resource, i) => (
              <li key={i} className="flex items-start gap-4">
                <CheckCircle className="w-6 h-6 text-amber-600 dark:text-amber-400 shrink-0 mt-1" />
                <div>
